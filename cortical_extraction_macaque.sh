#!/bin/bash
export BrainSuiteDir=/home/ajoshi/Software/BrainSuite23a
set -e
if [ -z "$1" ]; then
  echo "BrainSuite v23a cortical surface extraction script"
...
if [ -z $BrainSuiteDir ]; then
	BrainSuiteBin="$( cd -P "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"/
	BrainSuiteDir=$(dirname "$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )")/
elif [ -z $BrainSuiteBin ]; then
	BrainSuiteBin="${BrainSuiteDir}/bin/"
fi;
echo "Using BrainSuite data in $BrainSuiteDir"
echo "Using BrainSuite binaries in $BrainSuiteBin"
...
if [ $? -ne 0 ]; then echo "cortical extraction halted because scrubmask failed to run or returned an error."; exit 1; fi;
"${BrainSuiteBin}tca" $VERBOSE -i "${basename}.cortex.scrubbed.mask.$EXT" -o "${basename}.cortex.tca.mask.$EXT" -m 2500  --delta 20 $TCASETTINGS
if [ $? -ne 0 ]; then echo "cortical extraction halted because tca failed to run or returned an error."; exit 1; fi;
"${BrainSuiteBin}dewisp" $VERBOSE -i "${basename}.cortex.tca.mask.$EXT" -o "${basename}.cortex.dewisp.mask.$EXT" $DEWISPSETTINGS
if [ $? -ne 0 ]; then echo "cortical extraction halted because dewisp failed to run or returned an error."; exit 1; fi;
"${BrainSuiteBin}dfs" $VERBOSE -i "${basename}.cortex.dewisp.mask.$EXT" -o "${basename}.inner.cortex.dfs" -n 10 -a 0.5 -w 5.0
if [ $? -ne 0 ]; then echo "cortical extraction halted because dfs failed to run or returned an error."; exit 1; fi;
...
if [ $? -ne 0 ]; then echo "cortical extraction halted because pialmesh failed to run or returned an error."; exit 1; fi;
...
if [ $? -ne 0 ]; then echo "cortical extraction halted because hemisplit failed to run or returned an error."; exit 1; fi;
