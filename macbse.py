...
def main():
    ...
    parser.add_argument("-i", "--input", help="Input filename (MRI image filename)", required=True)
    parser.add_argument("-m", "--model", help="Model file (Trained model .pth file)", required=True)
    parser.add_argument("-o", "--output", help="Output filename (Skull extracted image filename)", required=True)
    ...
    channels = (2,8,8,16,32) #(16, 64, 64, 128, 256) #tuple(map(int, last_subdirectory.split('_')[1][1:-1].split(',')))
    ...
    estimated_mask = Resize(mode='trilinear', spatial_size=orig_shape)(estimated_mask[None,])[0]
    ...
    if mask_filename is not None:
        mask_nifti = nib.Nifti1Image((estimated_mask).numpy().astype(np.single), input_nifti.affine)
